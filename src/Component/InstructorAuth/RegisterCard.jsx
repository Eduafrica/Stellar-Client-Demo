                        ...
                        <div className="w-full flex flex-col gap-[24px]">
                           <div className="w-full flex flex-col gap-5">
                                <div className="inputGroup">
                                    ...
                                    <input type="text" id='name' onChange={handleChange} value={formData?.name || ''} placeholder='Enter your Full name' className="input" />
                                </div>
                                <div className="inputGroup">
                                    <label className="label">Email*</label>
                                    <input type="email" id='email' onChange={handleChange} value={formData?.email || ''} placeholder='Enter your email' className="input" />
                                </div>
                                <div className="inputGroup">
                                    <label className="label">Display Name*</label>
                                    <input type="text" id='displayName' onChange={handleChange} value={formData?.displayName || ''} placeholder='Enter your display name' className="input" />
                                </div>
                                <div className="inputGroup">
                                    <label className="label">Password*</label>
                                    <div className="relative w-full">
                                        <input type={ showPassword ? 'password' : 'text'} id='password' onChange={handleChange} value={formData?.password || ''} placeholder='Create a passowrd' className="input" />
                                        <div onClick={() => setShowPassword(!showPassword)} className="text-gray-500 cursor-pointer absolute right-[8px] top-1/4 bg-white">{ showPassword ? 'Show' : 'Hide'}</div>
                                    </div>
                                </div>
                           </div>

                           {/**BUTTON */}
                           <div onClick={handleRegister} className={`btn`}>{ loading ? 'Please Wait...' : 'Get Started' }</div>
                        </div>

                    </div>

                    <div className="text-[12px] text-text-secondary font-medium">
                        Already have an account? <Link to='/instructor/login' className='text-primary'>Go here</Link>
                    </div>
